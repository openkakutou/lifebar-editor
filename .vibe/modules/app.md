# Module: app
**Role:** Application entry point. Mounts the app's root frame — a `web-ui-kit` `<wuik-app-shell>` with the app title (`<h1>`) and version, plus the lifebar file input, the sprite sheet input, and the elements editor as main content — into the DOM, or a dependency-free error state if the design tokens stylesheet failed to load. Owns the `Set` of expanded elements-editor section indices, re-rendering the editor (with that same `Set`) after either the lifebar document or the sprite sheet store changes.
